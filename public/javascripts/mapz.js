...
function initMap (markers, maptitle, layertype, vector ) {
  info = $("#infopane");
  ...
  */
  ...
  $.each(markers, function(i,m){
    ...
    var marker = L.marker([mark.lat, mark.lon],{
      icon: L.divIcon({className: 'map_icon mic i_'+mark.clr+' s_1 id_'+mark.id}),
      riseOnHover: true,
    }).addTo(markerLayer);
    marker.id = mark.id;
...
function updateInfo (type, content) {
  ...
  if (type == 0 || type == undefined) {
    ...
    info.html("<div class='infocontent'>"+content+"</div>"+legend);
  }
};
