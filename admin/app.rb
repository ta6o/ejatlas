...
class Admin < Padrino::Application
  ...
  before do
    ...
    if current_account
      puts "#{request.xhr? ? "XHR ".green : "    "}#{request.request_method.to_s.cyan} #{request.url.blue} FROM #{request.ip.magenta}(#{current_account.email.yellow}) ON #{request.user_agent.gsub(/\([^\)]+\)/,"#|#").split("#|#")[0].strip.red} AT #{Time.now.strftime("%Y%m%d%H%M%S")[2..-1].yellow} #{ params.keys.any? and request.request_method != "GET" ? "WITH #{params.keys.to_s.green}" : ""}" unless request.path_info == "/error"
    else
      puts "#{request.xhr? ? "XHR ".green : "    "}#{request.request_method.to_s.cyan} #{request.url.blue} FROM #{request.ip.magenta} ON #{request.user_agent.gsub(/\([^\)]+\)/,"#|#").split("#|#")[0].strip.red} AT #{Time.now.strftime("%Y%m%d%H%M%S")[2..-1].yellow} #{ params.keys.any? and request.request_method != "GET" ? "WITH #{params.keys.to_s.green}" : ""}" unless request.path_info == "/error"
    end
  end
