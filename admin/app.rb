...
class Admin < Padrino::Application
  ...
  before do
    ...
    puts "#{Time.now.strftime("%Y%m%d%H%M%S%L")[2..-1].yellow} #{request.xhr? ? "X ".red : "  "}#{request.request_method.to_s.cyan} #{request.url.colorize(color)} FROM #{current_account ? "#{current_account.email.green}-" : ""} #{request.ip.magenta} ON #{platform.red} BY #{agent.red} #{ params.keys.any? and request.request_method != "GET" ? "WITH #{params.keys.to_s.green}" : ""}"
  end
