...
=end
  ...
  get :index do
    if current_account
      if ["admin","editor"].include? current_account.role
        @conflicts = Admin.filter("{\"must_not\":{\"term\":{\"approval_status\":\"deleted\"}}}", true, "id,name,slug,edited_by,category_id,saved_at,approval_status".split(","),false,"conflict","saved_at","asc").map{|x| x["_source"]}
  ...
  get :approved do
    if current_account
      if ["admin","editor"].include? current_account.role
        @conflicts = Admin.filter("{}", true, "id,name,slug,edited_by,category_id,saved_at,approval_status".split(","),true,"conflict","saved_at","asc").map{|x| x["_source"]}
        @accounts = Admin.filter("{}", true, "id,name".split(","),false,"account").map{|x| [x["_source"]["id"], x["_source"]["name"]]}.to_h
        @categories = Category.all.map {|c| [c.id,c.name]}.to_h
        @conflicts.sort_by! {|c| ( c["updated_at"] || Time.now ) }
        @conflicts.reverse!
