...
Admin.controller do
  ...
  get :index do
    ...
    @feats = Featured.select('id, description, name, slug, image, headline, published').where(:published=>true).order("created_at desc")#.limit(6)
  ...
  get "/info/:id" do
    ...
    target = ""
    target = " target='_blank'" if request.referer.match(/\/embed/)
